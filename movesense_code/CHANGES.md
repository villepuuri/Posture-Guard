## Version 2.3.2.42c0d01 ##

- Bugfix: some GSP responses have no reference number
- Bugfix: Error 404 causes Logbook return 409
...
### Notes:
- ECGRR module uses now the same samplerate that ECG has been subscribed with. If there is no ongoing ECG subscription, the ECGRR chooses 125Hz as the sample rate.
- The "Wipe datamem on DFU" results an up-to 30 second delay when sensor is set to DFU mode. During the wipe the LED blinks rapidly indicating the data wipe.

## Version 2.3.1.f6eb243 ##

### Whats new:
- Feature: Optional datawipe areas on DFU
- Feature: Encrypted CustomGattSvc characteristics
- Feature: ECGRR Multi-samplerate support

- Bugfix: CustomGATTService cuts some incoming data
- Bugfix: Fast xfer fails after error
- Bugfix: Mem-leak on dual BLE WB re-connect
- Bugfix: GSP doesn't work with dual BLE
- Bugfix: BLE Disconnects when pairing 4.2 device
- Bugfix: Windows goes to connect-disconnect cycle
